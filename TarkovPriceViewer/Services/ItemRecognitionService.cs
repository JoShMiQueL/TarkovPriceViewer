...
namespace TarkovPriceViewer.Services
{
    ...
    {
        TarkovDevAPI.Item MatchItemName(string name, string name2, TarkovDevAPI.Data data);
    }

    public class ItemRecognitionService : IItemRecognitionService
    {
        public TarkovDevAPI.Item MatchItemName(string name, string name2, TarkovDevAPI.Data data)
        {
            if (data == null || data.items == null)
            {
                return new TarkovDevAPI.Item();
            }

            char[] itemname = (name ?? string.Empty).ToLower().Trim().ToCharArray();
            char[] itemname2 = (name2 ?? string.Empty).ToLower().Trim().ToCharArray();

            var result = new TarkovDevAPI.Item();
            int d = 999;
            ...
            {
                result = new TarkovDevAPI.Item { name = "Encrypted Message" };
            }
